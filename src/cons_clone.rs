...
enum List<T> where T: Clone + Copy {
    Empty,
    NonEmpty(Cons<T>),
}

impl<T> List<T> where T: Clone + Copy {
    fn cons(t: T, tail: List<T>) -> Self {
    ...
    fn from_array(a: &[T]) -> Self {
        let mut result: List<T> = List::Empty;
        for i in a.iter().rev() {
    ...
    fn map<U>(&self, f: &dyn Fn(T) -> U) -> List<U> where U: Clone + Copy {
        match self {
    ...
    fn filter(&self, f: &dyn Fn(T) -> bool) -> Self {
        match self {
            List::Empty => List::Empty,
            List::NonEmpty(c) =>
                if f(c.car) {
    ...
    fn fold<U>(&self, base: U, op: &dyn Fn(T, U) -> U) -> U {
        match self {
            List::Empty => base,
            List::NonEmpty(c) =>
    ...
    fn reverse_builder(&self, accum: List<T>) -> Self {
        match self {
            List::Empty => accum,
...
#[derive(Debug, PartialEq)]
struct Cons<T> where T: Clone + Copy {
    car: T,
    cdr: Box<List<T>>,
}

impl<T> Cons<T> where T: Clone + Copy {
    fn new(car: T, cdr: List<T>) -> Self {
...
mod tests {
    ...
    fn nth() {
        ...
        assert_eq!(&it.head(), &Some(&0));
        assert_eq!(&it.tail().expect("!!").head(), &Some(&1));
        assert_eq!(&it.nth(0), &Some(&0));
        assert_eq!(&it.nth(1), &Some(&1));
        assert_eq!(&it.nth(2), &None);
        assert_eq!(&it.length(), &2);
        assert_eq!(&it.length_iter(), &2);
    }
